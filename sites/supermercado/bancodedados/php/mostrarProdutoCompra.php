<?php
    ...
    $buscaProduto = $conexao->query("SELECT produto.codigo as codigo, marca.nome as marca, tipoProduto.nome as tipoProduto, saborAroma.nome as saborAroma, tamanho.tamanho as tamanho, tamanho.medida as medida FROM precos INNER JOIN produto ON precos.id_produto = produto.codigo INNER JOIN fornecedor ON produto.id_fornecedor = fornecedor.codigo INNER JOIN marca ON produto.id_marca = marca.codigo INNER JOIN tipoProduto ON produto.id_tipoProduto = tipoProduto.codigo INNER JOIN saborAroma ON produto.id_saborAroma = saborAroma.codigo INNER JOIN tamanho ON produto.id_tamanho = tamanho.codigo INNER JOIN tipoSegmento ON produto.id_tipoSegmento = tipoSegmento.codigo WHERE precos.id_fornecedor = ".$_POST['id'] );
