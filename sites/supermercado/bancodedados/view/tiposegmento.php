...
<head>
	...
	<title>Gestão Supermercado - Tipo Segmento</title>
				...
				<div class="container-fluid p-0">
						...
						<div class="row">
							...
							<div class="col-12 col-lg-8">
                                <div class="card">
                                    <div class="card-header">
                                        <h5 class="card-title mb-0">Tipo segmento:</h5>
                                    </div>
                                    <div class="card-body">
										<input type="hidden" name="codigo" value="<?Php echo isset($_GET['codigoEditar']) ? $_GET['codigoEditar'] : '' ?>">
                                        <input type="text" class="form-control" placeholder="Digite o tipo do segmento" name="nome" value="<?Php echo isset($_GET['nomeEditar']) ? $_GET['nomeEditar'] : '' ?>" required>
                                    </div>
                                </div>
                            </div>
							<div class="col-12 col-lg-4">
                                <div class="card">
									...
									<div class="card-body">
										<select class="form-select mb-3" name="segmento" required>
											<option value="">Selecione o Segmento</option>
											<?php $buscaSegmento = $conexao->query("SELECT * FROM segmento");?>
											<?php while($segmentoBusca = $buscaSegmento->fetch()) { 
												$segmento = new Segmento($segmentoBusca['codigo'],$segmentoBusca['nome'])
												?>
												<option value="<?php echo $segmento->getCodigo(); ?>" <?php if(isset($_GET['segmentoEditar'])){ echo $segmento->getCodigo() == $_GET['segmentoEditar'] ? 'selected' : '';}; ?>><?php echo $segmento->getNome(); ?></option>
										<?php } ?>
									</select>
								</div>
							</div>
							<div class="col-12 col-lg-12" style="text-align:right;">
							<?php echo botao("cadastroTipoSegmento")?>
							</div>
						</div>
					</form>
					<form action="#" method="get" style="margin-top: 20px; margin-bottom: 20px;">
						...
						<?php 
							...
							if($procura == ''){
								$sql = "SELECT * FROM tipoSegmento";
							}else{
								$sql = "SELECT tipoSegmento.codigo, tipoSegmento.nome, tipoSegmento.id_segmento FROM tipoSegmento INNER JOIN segmento ON tipoSegmento.id_segmento = segmento.codigo WHERE tipoSegmento.nome LIKE '%".$procura."%' or segmento.nome LIKE '%".$procura."%'";
							}
							$busca = $conexao->query($sql);
							$dados = array();
							while($row = $busca->fetch()){
								$dados[] = ['codigo' => $row['codigo'],'nome' => $row['nome'],'segmento' => $row['id_segmento']];
							}
						?>
						...
						<table class="table">
							...
							<tbody>
								...
								<?php $tipoSegmento = new TipoSegmento($row['codigo'], $row['nome'], $row['segmento'])?>
										<tr>
											<td><?php echo $tipoSegmento->getNome(); ?></td>
											<td><?php echo mostrarTabela($tipoSegmento->getSegmento(),'segmento','nome',$conexao); ?></td>
											<td>
												<form action="#" method="get">
													<input type="hidden" name="codigoEditar" value="<?php echo $tipoSegmento->getCodigo(); ?>">
													<input type="hidden" name="nomeEditar" value="<?php echo $tipoSegmento->getNome(); ?>">
													<input type="hidden" name="segmentoEditar" value="<?php echo $tipoSegmento->getSegmento(); ?>">
