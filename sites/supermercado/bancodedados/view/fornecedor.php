...
<head>
	...
	<title>Gestão Supermercado - fornecedor</title>
			...
			<main class="content">
				...
				<div class="container-fluid p-0">
					...
					<form action="../controller/registrar.php" method="GET">
						<div class="row">
							<h1>Fornecedor <span id="mensagem" onmouseover="mostrarInformacoes('Cadastre os fornecedores.<br>Ex. Nestle, Ambev etc...')" onmouseout="tirarInformacoes()" style="background-color: red; padding: 2px 10px; border-radius: 50%;">?</span></h1>
							<div class="card">
								...
								<div class="card-body">
									...
									<input type="text" class="form-control" placeholder="Digite o nome do fornecedor" name="nome" value="<?Php echo isset($_GET['nomeEditar']) ? $_GET['nomeEditar'] : '' ?>" required>
								</div>
							</div>
							<div class="col-12 col-lg-12" style="text-align:right;">
								<?php echo botao("cadastroFornecedor")?>
							</div>
						</div>
					</form>
					<form action="#" method="get" style="margin-top: 20px; margin-bottom: 20px;">
						...
						<?php 
							...
							if($procura == ''){
								...
								$sql = "SELECT * FROM fornecedor WHERE nome LIKE '%".$procura."%'";
							}
						...
						<table class="table">
							...
							<tbody>
								<?php foreach($dados as $row){ ?>
									<?php $fornecedor = new fornecedor($row['codigo'], $row['nome'])?>
										<tr>
											<td><?php echo $fornecedor->getNome(); ?></td>
											<td>
												<form action="#" method="get">
													<input type="hidden" name="codigoEditar" value="<?php echo $fornecedor->getCodigo(); ?>">
													<input type="hidden" name="nomeEditar" value="<?php echo $fornecedor->getNome(); ?>">
											...
											<td>
												<?php echo botaoTabelaDeletar($fornecedor->getCodigo())?>
											</td>
										</tr>
								<?php } ?>
							</tbody>
