				...
				<div class="container-fluid p-0">
					...
					<?php 
							...
							if($procura == ''){
								...
								$sql = "SELECT promocao.codigo, promocao.id_fornecedor, promocao.id_produto, promocao.preco, promocao.dataInicio, promocao.dataFim FROM promocao INNER JOIN fornecedor ON promocao.id_fornecedor = fornecedor.codigo INNER JOIN produto ON promocao.id_produto = produto.codigo INNER JOIN marca ON produto.id_marca = marca.codigo INNER JOIN tipoProduto ON produto.id_tipoProduto = tipoProduto.codigo INNER JOIN saborAroma ON produto.id_saborAroma = saborAroma.codigo INNER JOIN tamanho ON produto.id_tamanho = tamanho.codigo WHERE fornecedor.nome LIKE '%".$procura."%' or promocao.preco LIKE '%".$procura."%' or promocao.dataInicio LIKE '%".$procura."%' or promocao.dataFim LIKE '%".$procura."%' or marca.nome LIKE '%".$procura."%' or tipoProduto.nome LIKE '%".$procura."%' or saborAroma.nome LIKE '%".$procura."%' or tamanho.tamanho LIKE '%".$procura."%' or tamanho.medida LIKE '%".$procura."%'";
							}
							$busca = $conexao->query($sql);
							$dados = array();
							while($row = $busca->fetch()){
								$dados[] = ['codigo' => $row['codigo'],'id_fornecedor' => $row['id_fornecedor'],'id_produto' => $row['id_produto'],'preco' => $row['preco'],'dataInicio' => $row['dataInicio'],'dataFim' => $row['dataFim']];
							}
						?>
						...
						<table class="table">
							...
							<tbody>
								<?php foreach($dados as $row){ ?>
									<?php $promocao = new Promocao($row['codigo'], $row['id_fornecedor'], $row['id_produto'], $row['preco'], $row['dataInicio'], $row['dataFim'])?>
										<tr>
											<td><?php echo mostrarTabela($promocao->getFornecedor(),'fornecedor','nome',$conexao); ?></td>
											<td><?php echo mostrarTabelaProduto($promocao->getProduto(),'produto','marca','tipoProduto','saborAroma','tamanho','medida',$conexao); ?></td>
