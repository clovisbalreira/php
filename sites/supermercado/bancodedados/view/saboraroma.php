...
<head>
	...
	<title>Gestão Supermercado - Sabor/Aroma</title>
			...
			<main class="content">
				...
				<div class="container-fluid p-0">
					...
					<form action="../controller/registrar.php" method="GET">
						<div class="row">
							<h1>Sabor/Aroma <span id="mensagem" onmouseover="mostrarInformacoes('Cadastre os sabores ou aromas dos produtos.<br>Ex. Laranja, Morango etc...')" onmouseout="tirarInformacoes()" style="background-color: red; padding: 2px 10px; border-radius: 50%;">?</span></h1>
							...
							<div class="col-12 col-lg-12">
                                <div class="card">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <input type="text" class="form-control" placeholder="Digite o sabor ou aroma" name="nome" value="<?Php echo isset($_GET['nomeEditar']) ? $_GET['nomeEditar'] : '' ?>" required>
                                    </div>
                                </div>
                            </div>
							<div class="col-12 col-lg-12" style="text-align:right;">
								<?php echo botao("cadastroSaborAroma")?>
							</div>
						</div>
					</form>
					<form action="#" method="get" style="margin-top: 20px; margin-bottom: 20px;">
						...
						<?php 
							...
							if($procura == ''){
								$sql = "SELECT * FROM saborAroma";
							}else{
								$sql = "SELECT * FROM saborAroma WHERE nome LIKE '%".$procura."%'";
							}
							$busca = $conexao->query($sql);
							$dados = array();
							while($row = $busca->fetch()){
								$dados[] = ['codigo' => $row['codigo'],'nome' => $row['nome']];
							}
						?>
						...
						<table class="table">
							...
							<tbody>
								...
								<?php $saborAroma = new SaborAroma($row['codigo'], $row['nome'])?>
										<tr>
											<td><?php echo $saborAroma->getNome(); ?></td>
											<td>
												<form action="#" method="get">
													<input type="hidden" name="codigoEditar" value="<?php echo $saborAroma->getCodigo(); ?>">
													<input type="hidden" name="nomeEditar" value="<?php echo $saborAroma->getNome(); ?>">
											...
											<td>
												<?php echo botaoTabelaDeletar($saborAroma->getCodigo())?>
											</td>
										</tr>
								<?php } ?>
							</tbody>
