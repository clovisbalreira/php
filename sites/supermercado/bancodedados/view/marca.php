...
<head>
	...
	<title>Gestão Supermercado - Marca</title>
					...
					<form action="#" method="get" style="margin-top: 20px; margin-bottom: 20px;">
						...
						<?php 
							...
							if($procura == ''){
								...
								$sql = "SELECT marca.codigo, marca.nome, marca.id_fornecedor FROM marca INNER JOIN fornecedor ON marca.id_fornecedor = fornecedor.codigo WHERE marca.nome LIKE '%".$procura."%' or fornecedor.nome LIKE '%".$procura."%'";
							}
							$busca = $conexao->query($sql);
							$dados = array();
							while($row = $busca->fetch()){
								$dados[] = ['codigo' => $row['codigo'],'nome' => $row['nome'],'fornecedor' => $row['id_fornecedor']];
							}
						?>
						...
						<table class="table">
							...
							<tbody>
								<?php foreach($dados as $row){ ?>
									<?php $marca = new marca($row['codigo'], $row['nome'], $row['fornecedor'])?>
										<tr>
											<td><?php echo $marca->getNome(); ?></td>
											<td><?php echo mostrarTabela($marca->getFornecedor(),'fornecedor','nome',$conexao); ?></td>
											<td>
