...
<head>
	...
	<title>Gestão Supermercado - Setor</title>
							...
							<div class="col-12 col-lg-12">
								<div class="card">
									...
									<div class="card-body">
										...
										<input type="text" class="form-control" placeholder="Digite o nome do setor" name="nome" value="<?Php echo isset($_GET['setorEditar']) ? $_GET['setorEditar'] : '' ?>" required>
									</div>
								</div>						
							</div>
							<div class="col-12 col-lg-12" style="text-align:right;">
								<?php echo botao("cadastroSetor")?>
							</div>
						</div>
					</form>
					<form action="#" method="get" style="margin-top: 20px; margin-bottom: 20px;">
						...
						<?php 
							...
							if($procura == ''){
								...
								$sql = "SELECT * FROM setor WHERE setor LIKE '%".$procura."%'";
							}
							$busca = $conexao->query($sql);
							$dados = array();
							while($row = $busca->fetch()){
								$dados[] = ['codigo' => $row['codigo'],'nome' => $row['setor']];
							}
						?>
							...
							<table class="table">
								...
								<tbody>
									<?php foreach($dados as $row){ ?>
										<?php $setor = new setor($row['codigo'], $row['nome'])?>
											<tr>
												<td><?php echo $setor->getNome();?></td>
												<td>
													<form action="#" method="get">
														<input type="hidden" name="codigoEditar" value="<?php echo $setor->getCodigo();?>">	
														<input type="hidden" name="setorEditar" value="<?php echo $setor->getNome();?>">
